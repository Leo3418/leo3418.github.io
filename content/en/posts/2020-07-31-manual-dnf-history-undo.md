...
package was matched? It was because transaction 217 ([log]({{< static-path res
transaction-217.txt >}})) installed Wine 5.10, but transaction 229 ([log]({{<
static-path res transaction-229.txt >}})), which updated my system, upgraded
Wine to 5.12. `dnf history undo` identifies packages by combinations of package
name, version and architecture instead of merely the names, so it does not
treat `wine-5.10-1.fc32.x86_64` and `wine-5.12-1.fc32.x86_64` as equivalent
packages.
...
in that transaction in its [output]({{< static-path res transaction-217.txt >}}). I
