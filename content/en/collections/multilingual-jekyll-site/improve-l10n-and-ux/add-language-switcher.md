...
added** to `href=" {{ page.url }}"`, between the quotation mark and the left
curly brace. This space is used to disable Polyglot's [URL
relativizing](https://github.com/untra/polyglot/blob/1.3.2/README.md#relativized-local-urls):
without it, Polyglot will still prepend the active language code `/{{ lang }}`
to the URL, which effectively creates a link to the same webpage, in the same
language.
...
by with `{% include lang-switcher.html %}`.
...
![Changing language to English]({{< static-path img footer-en.png >}})

![Changing language to Chinese]({{< static-path img footer-zh.png >}})
