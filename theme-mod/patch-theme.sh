...
# Array of additional theme files that should be patched, even if they do not
# contain matches of any PATTERNS
ADDITIONAL_FILES=(
    "_includes/sidebar.html"
)

# Array of directories in the site root that contain theme files
...
if (grep -Fqr -- "${THEME_FILE_LABEL}" "${DIRS_WITH_THEME_FILES[@]}"); then
    ...
    grep -FZlr -- "${THEME_FILE_LABEL}" "${DIRS_WITH_THEME_FILES[@]}" | \
        xargs -0 rm --
fi

echo "Getting files need to be patched..."
grep --exclude='*.md' -EZlr -- "${PATTERNS}" "${THEME_PATH}" | \
    xargs -0 cp --parents -t . --
